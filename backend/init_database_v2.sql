...
CREATE POLICY "users_update_name_only" ON users FOR UPDATE TO authenticated
USING (auth.uid() = id)
WITH CHECK (auth.uid() = id AND role = (SELECT role FROM users WHERE id = auth.uid()) AND verification_status = (SELECT verification_status FROM users WHERE id = auth.uid()) AND email = (SELECT email FROM users WHERE id = auth.uid()));

DROP POLICY IF EXISTS "users_no_delete" ON users;
CREATE POLICY "users_no_delete" ON users FOR DELETE TO authenticated USING (false);
...
CREATE POLICY "products_insert_authenticated" ON products FOR INSERT TO authenticated WITH CHECK (auth.uid() = seller_id);
...
CREATE POLICY "orders_select_own" ON orders FOR SELECT TO authenticated USING (auth.uid() = buyer_id);

DROP POLICY IF EXISTS "orders_insert_own" ON orders;
CREATE POLICY "orders_insert_own" ON orders FOR INSERT TO authenticated WITH CHECK (auth.uid() = buyer_id);

DROP POLICY IF EXISTS "order_items_select_own" ON order_items;
CREATE POLICY "order_items_select_own" ON order_items FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM orders WHERE orders.id = order_items.order_id AND orders.buyer_id = auth.uid()));
...
CREATE POLICY "verification_select_own" ON verification_documents FOR SELECT TO authenticated USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "verification_insert_own" ON verification_documents;
CREATE POLICY "verification_insert_own" ON verification_documents FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "invite_codes_select_unused" ON merchant_invite_codes;
CREATE POLICY "invite_codes_select_unused" ON merchant_invite_codes FOR SELECT TO authenticated, anon USING (is_used = false);
